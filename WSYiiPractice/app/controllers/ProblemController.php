    ...
    public function actionSolve($id)
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) { 
                ...
                $newFileName = md5($model->photo_after->baseName. '.'. $model->photo_after->extension.time()) .'.'. $model->photo_after->extension;
                $model->photo_after->saveAs('@webroot/uploads/'.$newFileName);
